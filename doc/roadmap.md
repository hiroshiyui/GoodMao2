...
[goodmao.tw](https://goodmao.tw); that chapter is a record of what was built, not a checklist.
...
engineering/ops, and accessibility tranches. **Released 2026-07-23**; what was consciously
deferred past it now lives in [Milestone v1.1.0](#milestone-v110).

Everything in this chapter shipped, so the entries carry no checkboxes — a page of `[x]`
reads as a form to fill in rather than a record of what was built. The open work is checkboxed
in v1.1.0, where the state still varies.
...
- Scope-based auth (`phx.gen.auth`), first user → administrator, editable `@handle`
- Administrator site-overview page (`/admin`, `:require_admin`-gated, IDOR-hidden) — a
...
- Pets: create / list / view / edit, coat colour, weight unit
- Owner-only end-of-care lifecycle (status transition, backdatable `ended_at`, reversible)
- `history_hidden` opt-in flag — schema, changeset, **and** read/write enforcement
  (existence-hidden timeline; see [ADR-0003](adr/0003-pet-lifecycle.md))
- Resource-based per-pet authorization (`owner` / `co_caretaker` / `viewer` / `vet`,
  capability levels, time-boxed grants, ≥1-owner invariant, IDOR-hidden 404s)
- Grant / revoke access by `@handle` or email (Sharing page)
- Structured log entries (single table + `type` + `jsonb`), per-type validation
- One-tap QuickLog (food / water / bathroom / vomit / weight / energy / medication / symptom)
- Backdatable `occurred_at`, free-text note, per-entry `visibility` — owner-only change
  **and** read-side `private` filtering (reads + live PubSub; see
  [ADR-0004](adr/0004-log-visibility.md))
- Vet-authored `vet_note` entries (vet-only)
- Live, type-filterable timeline via Phoenix PubSub — as a chronological list **or** a
  month **calendar view** (per-day counts, urgent/watch cues, day drill-down)
- Soft-delete of entries (`deleted_at`)
- Gettext throughout; `en` populated, `zh_TW` / `ja_JP` scaffolded
- Test suite (context + LiveView) and `mix precommit` gate; dev seed data
...
- **Enforce `history_hidden`** on every `Logs` read and write — when hidden the timeline
...
- **Per-entry `private` visibility on reads** — a caller sees a `private` entry only when
  ...
  [ADR-0004](adr/0004-log-visibility.md)).
- **≥1-owner invariant on the grant-update/expiry path** — `grant_access` now guards
  against demoting or time-boxing the last effective owner (`lib/goodmao2/pets.ex`).
- **Recorder-or-owner check on log edit/delete** — owner → any entry; anyone else → only
  what they recorded; `vet_note` edits stay vet-only (`lib/goodmao2/logs.ex`).
- **Site-owner registration gate** — optional `config :goodmao2, :site_owner_email`
  (env `GOODMAO_SITE_OWNER_EMAIL`); when set, only that address may create the first
  (admin) account (`lib/goodmao2/accounts.ex`).
- **Handle-rule parity** — the handle must start with a letter or number (leading `_`
  rejected) and the reserved-word set is expanded (`lib/goodmao2/accounts/user.ex`).
- **Row-locked owner invariant** — owner-grant mutations run in a transaction that takes
...
- Weight trend chart (Phase 1) — an inline, CSP-safe SVG line chart of the pet's weight over
  ...
  over PubSub
- **Medication schedules + reminders** — the "did anyone give the pill?" coordination
...
- LifeLog media (photos/videos) with active purification ([ADR-0005](adr/0005-media-storage.md);
...
- **One-tap QuickLog buttons** — each common value is its own submit button (Food:
...
- **Web Push** ([ADR-0011](adr/0011-notifications-and-messaging.md) Stage 2) — OS-level push
...
- Per-entry **share links** (public token) + anonymous shared entry/media
...
- **Locale switcher + per-request locale** — `Goodmao2Web.Plugs.Locale` resolves
...
- **Trilingual catalogs populated** — every `default` UI string and Ecto `errors`
  message translated for `zh_TW` and `ja_JP`, localized to each culture (ADR-0002), with
  the locale-parity test green.
- **Vendored Roboto Slab + CJK-aware font stack** — Roboto Slab (Apache-2.0, self-hosted
...
- **Localized the `phx.gen.auth` LiveViews** — log-in / register / confirmation (and the
  ...
  locale.
- **Readable default size + font-size control** — the base reading size is 20px
...
- **Timezone-aware display & input** ([ADR-0018](adr/0018-timezone-display-policy.md)) —
...
- **Data-model polish bundle** — four refinements within the existing model (no new ADR):
...
- **Profile images (avatars)** for users and pets ([ADR-0020](adr/0020-profile-images.md)) —
...
- **CI** (`.github/workflows/ci.yml`) — a `mix` job on a `postgres` service, Erlang/Elixir
  pinned from `.tool-versions`, running unused-deps + compile-warnings + format + audit +
  Sobelow + tests.
- **Dependabot** (`.github/dependabot.yml`) — `mix` + `github-actions`, weekly/grouped.
  (`npm` omitted — assets use the esbuild/tailwind installers, no `package.json`.)
- **`mix_audit` + `sobelow`** wired into `mix precommit` and CI.
- **`/health` endpoint + test** — `GET /health` returns `200 ok` when the DB is reachable.
- **Hard-fence `seeds.exs` to `:dev`** — refuses to run outside development.
- **`CHANGELOG.md`** — Keep-a-Changelog, version single-sourced in `mix.exs`.
- **Content-Security-Policy** on the browser pipeline — set per request by
...
- **Two-factor authentication** ([ADR-0013](adr/0013-second-factor-authentication.md)) —
...
- **`mix goodmao.doctor` preflight task** — checks Erlang/Elixir vs `.tool-versions`, Postgres
...
- **Co-hosting deployment note** (`doc/deployment.md`) — the manual runbook for running
...
- **Ansible-driven deployment** (`ansible/`) — provisioning + releases automated, mirroring
...
- **`:focus-visible` brand ring** (2 px + 2 px offset, `var(--color-primary)`) on every
  focusable element — backs the a11y invariant `AGENTS.md` states.
- **`aria-hidden` on decorative `<.icon>` glyphs** — the shared `<.icon>` now hides the glyph
...
- **Global `prefers-reduced-motion` guard** — neutralises transitions/animations and the
  micro-interactions/glow below (keeps the focus ring, which isn't motion).
- **Fluent design tokens** — layered elevation shadow ramp (`--gm-elevation-1..3`, deepened in
  dark), decelerating curve + durations, and opt-in `.gm-lift` (card hover-lift) / `.gm-press`
  (button press-depth) utilities.
- **`theme-color` meta + inline SVG favicon + branded `<.live_title>`** — the title suffix now
  reads "· GoodMao"; light/dark `theme-color`s match the base canvas.
- **Footer + sticky app-shell** — `#app-shell` flex column with a sticky, backdrop-blurred
  header and a `#site-footer` (the natural home for the future locale switcher).
- **Reveal pointer-glow** — the `PointerGlow` LiveView hook + `.gm-glow` surface; the hook
  never attaches its listener under `prefers-reduced-motion`.

- **Installable PWA** — a web app manifest + maskable icons, a root-scope service worker,
...
partially-shipped item in §8; and the **deployment tranche** (§9) — the co-hosting note (`doc/deployment.md`),
