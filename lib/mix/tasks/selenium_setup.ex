defmodule Mix.Tasks.Selenium.Setup do
  ...
  the matching `ARG`s in `.github/workflows` / the CI image, if one pins these.
  ...
  `Goodmao2Web.SeleniumServer` so the version lives in one place.
  """
  ...
  defp build_geckodriver(dest) do
    ...
    build_root =
      Path.join(System.tmp_dir!(), "goodmao2-geckodriver-#{System.unique_integer([:positive])}")
